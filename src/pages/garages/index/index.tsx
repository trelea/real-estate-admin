...
import { useGetGaragesQuery } from "@/features/garages/api";
...
import { useDeleteGarage } from "@/features/garages/hooks/use-delete-garage";

interface Props {
  status: User;
}

export const IndexGarages: React.FC<Props> = ({}) => {
  const redirect = useNavigate();
  const { currentLang } = useChangeLanguage();
  const [page, setPage] = useQueryState("page", parseAsInteger.withDefault(1));
  const { data, isFetching, isLoading } = useGetGaragesQuery({
    page,
    limit: 10,
    search: "",
  });

  const [deleteGarage, isDeletingGarage] = useDeleteGarage();

  return (
    <section className="w-full h-full">
      <PropertiesTable<Garage>
        loading={isFetching || isLoading || isDeletingGarage}
        header={{
